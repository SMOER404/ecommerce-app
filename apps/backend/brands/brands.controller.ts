...
export class BrandsController {
  ...
  findAll(@Query() params: PaginationParams): Promise<PaginatedResponse<Brand>> {
  ...
  @Put(':id/image')
  @ApiOperation({ 
    summary: 'Загрузка изображения бренда',
    description: 'Обновляет URL изображения для существующего бренда'
  })
  @ApiResponse({ 
    status: 200, 
    description: 'Изображение бренда успешно обновлено',
    type: Brand
  })
  @ApiResponse({ 
    status: 404, 
  ...
  uploadImage(
    ...
    return this.brandsService.update(id, { image: uploadBrandImageDto.imageUrl });
  }
} 